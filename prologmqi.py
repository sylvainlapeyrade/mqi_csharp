    ...
    def __init__(
        ...
        if self._unix_domain_socket is not None:
            if os.name == "nt":
                ...
                raise ValueError(
                    "Must only provide one of: port or unix_domain_socket")
    ...
    def start(self):
        ...
        if self._launch_mqi:
            ...
            if self.pending_connections is not None:
                launchArgs += [
                    f"--pending_connections={str(self.pending_connections)}"]
            ...
            if self._unix_domain_socket is not None:
                if len(self._unix_domain_socket) > 0:
                    launchArgs += [
                        f"--unix_domain_socket={self._unix_domain_socket}"]
                else:
            ...
            self._stderr_reader = _NonBlockingStreamReader(
            ...
            if self._unix_domain_socket is None:
                ...
                if domain_socket == "":
                    raise PrologLaunchError(
                        "no Unix Domain Socket found in stdout")
            ...
            self._stdout_reader = _NonBlockingStreamReader(
...
class PrologThread:
    ...
    def start(self):
        ...
        # to connect, give it 3 rseconds and then fail since it is a really small
        ...
        if prolog_name(jsonResult) != "true":
            ...
            self.communication_thread_id, self.goal_thread_id = prolog_args(
                threadTerm)
    ...
    def query(self, value: str, query_timeout_seconds: float = None):
        ...
        value = value.rstrip("\n.")
        timeoutString = (
            "_" if query_timeout_seconds is None else str(
    ...
    def query_async(
        ...
        self._send(
            f"run_async(({value}), {timeoutString}, {findallResultsString}).\n")
        self._return_prolog_response()
...
def is_prolog_functor(json_term):
    ...
    return (
        isinstance(
            json_term, dict) and "functor" in json_term and "args" in json_term
    )
